-- Evita recursión infinita: las políticas de profiles no deben hacer SELECT sobre profiles.
-- is_staff() corre como dueño de la función y lee profiles sin reaplicar RLS.
...
-- Quitar políticas que causaban recursión (si existen)
...
-- profiles: staff ve filas de pacientes (sin subquery a profiles)
...
-- goals: staff + fila de paciente (subquery solo sobre user_id; is_staff no toca policies de profiles en bucle)
CREATE POLICY "goals_staff_select"
  ...
  USING (
    ...
    AND EXISTS (
      ...
      WHERE u.id = goals.user_id AND u.role = 'user'
    )
  );

CREATE POLICY "goals_staff_insert"
...
CREATE POLICY "meal_entries_staff_read_users"
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1 FROM public.profiles u
      WHERE u.id = meal_entries.user_id AND u.role = 'user'
    )
  );

CREATE POLICY "weekly_controls_staff_read"
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1 FROM public.profiles u
      WHERE u.id = weekly_controls.user_id AND u.role = 'user'
    )
  );

CREATE POLICY "body_metrics_staff_read"
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1 FROM public.profiles u
      WHERE u.id = body_metrics.user_id AND u.role = 'user'
    )
  );
